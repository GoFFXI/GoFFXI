...
func (s *InstanceWorker) CreateNATSConnection() error {
...
func (s *InstanceWorker) OnNATSDisconnected(_ *nats.Conn, err error) {
	s.Logger().Warn("NATS disconnected", "error", err)
	s.natsConn = nil
}

func (s *InstanceWorker) OnNATSReconnected(nc *nats.Conn) {
	s.Logger().Info("NATS reconnected")
	s.natsConn = nc
}

func (s *InstanceWorker) OnNATSClosed(_ *nats.Conn) {
